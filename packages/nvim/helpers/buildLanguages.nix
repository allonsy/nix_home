...
let
  ...
  languageCommandList = map (
    ...
    let
      ...
      actualRevision =
        if builtins.stringLength languageConf.revision != 40 then
          overrides.${language}.${languageConf.revision}
        else
          languageConf.revision;
      languageRepo = builtins.fetchGit ({
        name = language;
        url = languageConf.repo;
        rev = actualRevision;
      });
      lsp_config_line =
        if
        ...
        then
          "echo \"vim.lsp.enable('${languageLSPConfig.${language}.conf}')\" >> $out/conf.lua"
        else
          "";
      lsp_package_line =
        if
        ...
        then
          "cp ${pkgs.${languageLSPConfig.${language}.pkg}}/bin/${languageLSPConfig.${language}.pkg} $out/bin"
        else
          "";
    in
    ''
